...
class AdministratorController extends Controller
{
    public function index()
    {
        ...
        $dataMonev = Program::selectRaw('status_monev, COUNT(*) as total')
            ...
            ->get();
        return view('administrator.dashboard', compact('dataProgram','dataMonev'));
    }

    public function profile_index()
    {
        return view('administrator.profile_index');
    }
    ...
    // PENGATURAN AKUN SEKRETARIS/LITBANG/KEUANGAN-BENDAHARA

    public function pengaturan_akun()
    {
        $listUsersSeksi = User::with(['seksi'])->whereIn('role', [3, 4, 5])->get();

        $listSeksi = Seksi::all();
        $listSubSeksi = Sub_seksi::all();

        
        return view('administrator.pengaturan_akun',compact('listUsersSeksi', 'listSeksi', 'listSubSeksi'));
    }
}
